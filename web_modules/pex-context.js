...
function createRenderingContext(opts = {}) {
	const { width = window.innerWidth, height = window.innerHeight, pixelRatio = 1, fullscreen = !opts.width && !opts.height, type = "webgl", element = document.body, ...contextAttributes } = { ...opts };
	...
	if (!opts.canvas) {
		...
		const appendCanvas = () => {
			if (fullscreen) Object.assign(document.body.style, {
				margin: 0,
				overflow: "hidden",
				backgroundColor: "#000"
			});
	...
	for (let i = 0; i < contexts.length; i++) try {
...
function compareFBOAttachments(framebuffer, passOpts) {
	if (framebuffer.depth?.texture != (passOpts.depth?.texture || passOpts.depth)) return false;
	if (framebuffer.color.length != passOpts.color?.length) return false;
	for (let i = 0; i < framebuffer.color.length; i++) if (framebuffer.color[i]?.texture != (passOpts.color[i]?.texture || passOpts.color[i])) return false;
	return true;
}
...
function enableVertexData(ctx, vertexLayout, cmd, updateState) {
	...
	for (let i = vertexLayout.length; i < ctx.capabilities.maxVertexAttribs; i++) if (ctx.state.activeAttributes[i]) {
	...
	for (let i = 0; i < vertexLayout.length; i++) {
		...
		if (!buffer && attrib.class === "vertexBuffer") buffer = attrib;
		if (!buffer || !buffer.target) throw new Error(`Trying to draw arrays with invalid buffer for attribute : ${name}`);
		...
		if (size === 16) {
			...
			if (!ctx.capabilities.drawInstancedBase && (cmd.baseInstance || cmd.baseVertex)) {
				if (indices) offset += (attrib.divisor ? cmd.baseInstance : cmd.baseVertex) * size * ctx.DataTypeConstructor[type].BYTES_PER_ELEMENT;
				else if (attrib.divisor) offset += cmd.count * cmd.baseInstance / size;
			}
	...
	if (indices) {
		...
		if (!indexBuffer && indices.class === "indexBuffer") indexBuffer = indices;
...
function draw(ctx, cmd) {
	...
	if (cmd.indices || cmd.vertexArray?.indices) drawElements(ctx, cmd, instanced, primitive);
	else if (cmd.count || cmd.multiDraw?.counts) drawArrays(ctx, cmd, instanced, primitive);
	else throw new Error("Vertex arrays requires elements, count or multiDraw");
}
...
const isBuffer = (object) => ["vertexBuffer", "indexBuffer"].includes(object?.class);
...
function updateTexture(ctx, texture, opts) {
	...
	let internalFormat;
	if (opts.internalFormat) internalFormat = opts.internalFormat;
	else if (opts.pixelFormat) internalFormat = gl[pixelFormat];
	else internalFormat = texture.internalFormat ?? gl[pixelFormat];
	let type;
	let format;
	const textureUnit = 0;
	...
	if (!internalFormat || opts.internalFormat) {
		...
		if (ctx.gl instanceof WebGLRenderingContext) {
			if (ctx.capabilities.depthTexture && ["DEPTH_COMPONENT16", "DEPTH_COMPONENT24"].includes(pixelFormat)) internalFormat = gl["DEPTH_COMPONENT"];
			if (!internalFormat) {
				...
				} else if (pixelFormat === ctx.PixelFormat.RGBA16F || pixelFormat === ctx.PixelFormat.RGBA32F) internalFormat = gl.RGBA;
			}
		}
	...
	if (isElement(element) || !ctx.capabilities.isWebGL2 && element instanceof ImageBitmap) {
		...
		console.assert(!data || width !== void 0 && height !== void 0, "Texture2D.update opts.width and opts.height are required when providing opts.data");
	...
	} else throw new Error("Texture2D.update opts has to be a HTMLElement, ImageBitmap or Object");
...
function updateTexture2D(ctx, texture, data, { offset } = {}) {
	...
	for (let level = 0; level < data.length; level++) {
		...
		if (Array.isArray(levelData)) levelData = arrayToTypedArray(ctx, type, levelData);
		if (compressed) gl.compressedTexImage2D(target, level, internalFormat, width, height, 0, levelData);
...
function updateTextureCube(ctx, texture, data) {
	...
	for (let i = 0; i < 6; i++) {
		...
		if (isElement(faceData)) gl.texImage2D(faceTarget, lod, internalFormat, format, type, faceData);
		else {
			if (Array.isArray(faceData)) faceData = arrayToTypedArray(ctx, type, faceData);
...
function createFramebuffer(ctx, opts) {
	...
	if (opts.color || opts.depth) updateFramebuffer(ctx, framebuffer, opts);
	return framebuffer;
}
function updateFramebuffer(ctx, framebuffer, opts) {
	...
	framebuffer.color = opts.color ? opts.color.map((attachment) => {
		...
		if (!colorAttachment.target) colorAttachment.target = colorAttachment.texture.target;
	...
	framebuffer.color.forEach((colorAttachment, i) => {
		...
		if (colorAttachment.target === gl.RENDERBUFFER) gl.framebufferRenderbuffer(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0 + i, gl.RENDERBUFFER, colorAttachment.handle || colorAttachment.texture.handle);
		else gl.framebufferTexture2D(framebuffer.target, gl.COLOR_ATTACHMENT0 + i, colorAttachment.target, colorAttachment.texture.handle, colorAttachment.level);
	});
	for (let i = framebuffer.color.length; i < ctx.capabilities.maxColorAttachments; i++) gl.framebufferTexture2D(framebuffer.target, gl.COLOR_ATTACHMENT0 + i, gl.TEXTURE_2D, null, 0);
	if (framebuffer.depth) {
		if (ctx.debugMode) console.debug(NAMESPACE, "fbo attaching depth", framebuffer.depth);
		const depthAttachment = framebuffer.depth;
		if (depthAttachment.texture.target === gl.RENDERBUFFER) gl.framebufferRenderbuffer(gl.FRAMEBUFFER, gl.DEPTH_ATTACHMENT, gl.RENDERBUFFER, depthAttachment.texture.handle);
		else gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.DEPTH_ATTACHMENT, depthAttachment.texture.target, depthAttachment.texture.handle, depthAttachment.level);
	} else {
...
function updateRenderbuffer(ctx, renderbuffer, opts) {
	...
	if (opts.sampleCount) gl.renderbufferStorageMultisample(renderbuffer.target, Math.min(opts.sampleCount, ctx.capabilities.maxSamples), renderbuffer.format, renderbuffer.width, renderbuffer.height);
	else gl.renderbufferStorage(renderbuffer.target, renderbuffer.format, renderbuffer.width, renderbuffer.height);
...
function createPass(ctx, opts) {
	...
	if (opts.color || opts.depth) {
		...
		const colorsToResolve = opts.color ? opts.color.map((attachment, index) => {
			if (attachment.resolveTarget) return {
...
function createPipeline(ctx, opts) {
	...
	const pipeline = Object.assign({
		...
		_dispose() {
			this.vert = null;
			this.frag = null;
			if (this.program && --this.program.refCount === 0 && this.program.handle) ctx.dispose(this.program);
			this.program = null;
		}
	}, opts);
	if (opts.vert && opts.frag) pipeline.program = ctx.program({
		vert: opts.vert,
		frag: opts.frag,
...
function updateTransformFeedback(ctx, transformFeedback, opts) {
	...
	let index = 0;
	for (let varyingName in opts.varyings) gl.bindBufferBase(gl.TRANSFORM_FEEDBACK_BUFFER, index++, opts.varyings[varyingName].handle);
...
function createProgram(ctx, opts) {
	...
	const program = {
		...
		setUniform(name, value) {
			...
			if (uniform === void 0) throw new Error(`Uniform ${name} is not defined`);
			const gl = ctx.gl;
			const type = uniform.type;
			const location = uniform.location;
			const uniformMethod = ctx.UniformMethod[uniform.type];
			if (!uniformMethod) throw new Error(`Invalid uniform type ${type} : ${ctx.getGLString(type)}`);
			else if (uniformMethod.includes("Matrix")) gl[uniformMethod](location, false, value);
			else gl[uniformMethod](location, value);
		}
	};
...
function updateProgram(ctx, program, { vert, frag, vertexLayout }) {
	...
	if (vertexLayout) for (let [name, attribute] of Object.entries(vertexLayout)) gl.bindAttribLocation(program.handle, attribute.location, name);
	gl.attachShader(program.handle, vertShader);
	gl.attachShader(program.handle, fragShader);
	gl.linkProgram(program.handle);
	if (!gl.getProgramParameter(program.handle, gl.LINK_STATUS)) throw new Error(`Program: ${gl.getProgramInfoLog(program.handle)}`);
...
function compileSource(ctx, type, src) {
	...
	if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
		...
		if (ctx.debugMode) console.debug(NAMESPACE, `${shaderType} shader compilation failed`, src);
...
function updateUniforms(ctx, program) {
	...
	for (let i = 0; i < numUniforms; ++i) {
		...
		if (size === void 0) throw new Error(`Unknwon uniform type ${info.type} : ${ctx.getGLString(info.type)}`);
		...
		if (info.size > 1) for (let j = 1; j < info.size; j++) {
...
function updateBuffer(ctx, buffer, opts) {
	...
	if (Array.isArray(data)) {
		if (!type) {
			if (buffer.target === gl.ARRAY_BUFFER) type = ctx.DataType.Float32;
			else if (buffer.target === gl.ELEMENT_ARRAY_BUFFER) type = ctx.DataType.Uint16;
		}
		...
		else if (type === ctx.DataType.Int8) data = new Int8Array(elemSize === 1 ? sourceData : size);
		if (elemSize > 1) for (let i = 0; i < sourceData.length; i++) for (let j = 0; j < elemSize; j++) {
			const index = i * elemSize + j;
			data[index] = sourceData[i][j];
		}
	} else if (data instanceof Float32Array) type = ctx.DataType.Float32;
	else if (data instanceof Uint8Array) type = ctx.DataType.Uint8;
	else if (data instanceof Uint16Array) type = ctx.DataType.Uint16;
	else if (data instanceof Uint32Array) type = ctx.DataType.Uint32;
	else if (data instanceof Int8Array) type = ctx.DataType.Int8;
	...
	if (ctx.state.vertexArray) {
		ctx.state.vertexArray = void 0;
		gl.bindVertexArray(null);
	}
	gl.bindBuffer(buffer.target, buffer.handle);
	if (offset) gl.bufferSubData(buffer.target, offset, data);
	else gl.bufferData(buffer.target, data, buffer.usage);
}
...
function createQuery(ctx, opts = {}) {
	...
	if (!query.target) query.target = ctx.capabilities.disjointTimerQuery ? ctx.QueryTarget.TimeElapsed : ctx.QueryTarget.AnySamplesPassed;
	return query;
}
...
function available({ gl, QueryState }, q) {
	if (gl.getQueryParameter(q.handle, gl.QUERY_RESULT_AVAILABLE)) {
...
function polyfill(ctx) {
	...
	} else capabilities.maxColorAttachments = gl.getParameter(gl.MAX_COLOR_ATTACHMENTS);
	...
	*/
	ctx.TextureFormat = {
		...
		...Object.fromEntries([
			...
			"32UI": "Uint32",
			"32I": "Int32"
		}).map(([type, DataType]) => [[`${format}${type}`], [gl[`${format === "R" ? "RED" : format}${type.endsWith("I") ? "_INTEGER" : ""}`], ctx.DataType[DataType]]]))),
...
function createContext(options = {}) {
	...
	if (options.pixelRatio) opts.pixelRatio = Math.min(opts.pixelRatio, window.devicePixelRatio);
	...
	*/
	...
	const defaultState = {
		...
		framebuffer: void 0,
		indexBuffer: void 0,
		activeAttributes: void 0,
		activeTextures: void 0,
		vertexArray: void 0,
		vertexLayout: void 0,
		program: void 0,
		cullFace: void 0,
		cullFaceMode: void 0,
		blend: void 0,
		blendSrcRGBFactor: void 0,
		blendSrcAlphaFactor: void 0,
		blendDstRGBFactor: void 0,
		blendDstAlphaFactor: void 0,
		depthFunc: void 0,
		depthTest: void 0,
		depthWrite: void 0
	};
	Object.assign(ctx, {
		...
		getGLString(glEnum) {
			let str = "UNDEFINED";
			for (let key in gl) if (gl[key] === glEnum) {
				str = key;
				break;
			}
			return str;
		...
		*/
		set({ pixelRatio, width, height }) {
			if (pixelRatio) this.updatePixelRatio = Math.min(pixelRatio, window.devicePixelRatio);
		...
		frame(cb) {
			requestAnimationFrame(function frame() {
				if (this.updatePixelRatio) {
					...
					if (!this.updateWidth) this.updateWidth = parseInt(gl.canvas.style.width) || gl.canvas.width;
					if (!this.updateHeight) this.updateHeight = parseInt(gl.canvas.style.height) || gl.canvas.height;
				...
				if (this.isDisposed || cb() === false) return;
				if (this.queries.length) this.queries = this.queries.filter((q) => !q._available(this, q));
		...
		*/
		submit(cmd, batches, subCommand) {
			...
			if (cmd.pass?.resolveFramebuffer) this.resolvePass(cmd.pass);
			this.checkError();
		},
		program(opts) {
		...
		*/
		beginQuery(query) {
			...
			if (query._begin(this, query)) this.activeQuery = query;
		},
		/**
		...
		*/
		update(resource, opts) {
			if (this.debugMode) console.debug(NAMESPACE, "update", {
				resource,
				opts
			});
		...
		*/
		dispose(resource) {
			...
			if (resource) {
				if (!resource._dispose) console.assert(resource._dispose, "Trying to dispose non resource");
				...
				while (this.resources.length) this.dispose(this.resources[0]);
		...
		mergeCommands(parent, cmd, inherit) {
			...
			if (!inherit) {
				...
				newCmd.pass.clearColor = void 0;
				newCmd.pass.clearDepth = void 0;
			}
		...
		applyPass(pass) {
			...
			if (pass.framebuffer) {
				...
				if (framebuffer.id !== state.framebuffer.id) {
					if (this.debugMode) console.debug(NAMESPACE, "change framebuffer", state.framebuffer, "->", framebuffer);
					if (framebuffer._update && !compareFBOAttachments(framebuffer, pass.opts)) this.update(pass.framebuffer, pass.opts);
					ctx.state.framebuffer = framebuffer;
					gl.bindFramebuffer(framebuffer.target, framebuffer.handle);
					if (framebuffer.drawBuffers && framebuffer.drawBuffers.length > 1) gl.drawBuffers(framebuffer.drawBuffers);
				}
			} else {
				let framebuffer;
				let i = ctx.stack.length - 1;
				while (!framebuffer && i >= 0) {
					if (ctx.stack[i].pass) framebuffer = ctx.stack[i].pass.framebuffer;
					--i;
				}
		...
		resolvePass: (pass) => {
			...
			for (let i = startIndex; i < resolveFramebuffer.color.length; i++) {
				...
				if (i >= 0) {
					...
					if (resolveFramebuffer.color[i].texture.mipmap) ctx.update(pass.resolveFramebuffer.color[i].texture, { mipmap: true });
				}
			}
		...
		applyScissor(scissor) {
			...
			} else if (scissor !== this.state.scissor) {
				this.state.scissor = scissor;
		...
		applyClear(pass) {
			let clearBits = 0;
			if (pass.clearColor !== void 0) {
				if (this.debugMode) console.debug(NAMESPACE, "clearing color", pass.clearColor);
				const clearValues = pass.clearColor;
				const isClearBufferArray = is2DArray(clearValues);
				if (isWebGL2$1 && isClearBufferArray) for (let i = 0; i < clearValues.length; i++) {
					const color = clearValues[i];
					if (color) gl.clearBufferfv(gl.COLOR, i, color);
				}
				else {
					clearBits |= gl.COLOR_BUFFER_BIT;
					gl.clearColor(...isClearBufferArray ? clearValues[0] : clearValues);
				}
			}
			if (pass.clearDepth !== void 0) {
				if (this.debugMode) console.debug(NAMESPACE, "clearing depth", pass.clearDepth);
		...
		applyPipeline(pipeline) {
			...
			if (pipeline.cullFace !== state.cullFace || pipeline.cullFaceMode !== state.cullFaceMode) {
				...
				if (state.cullFace) gl.cullFace(state.cullFaceMode);
			}
			...
			if (pipeline.program !== state.program) {
				...
				if (state.program) gl.useProgram(state.program.handle);
			}
			if (pipeline.vertexLayout) state.vertexLayout = pipeline.vertexLayout;
			this.checkError();
		},
		checkActiveProgram() {
			if (!this.state.program) throw new Error("Trying to draw without an active program");
		},
		applyUniforms(uniforms, cmd) {
			...
			for (const name in uniforms) {
				...
				if (!program.uniforms[name] && !program.uniforms[`${name}[0]`]) continue;
				...
				if (Array.isArray(value) && !program.uniforms[name]) {
					if (this.debugMode) console.debug(NAMESPACE, "unknown uniform", name, Object.keys(program.uniforms));
					for (let i = 0; i < value.length; i++) {
						const nameIndex = `${name}[${i}]`;
						program.setUniform(nameIndex, value[i]);
						if (this.debugMode) requiredUniforms.splice(requiredUniforms.indexOf(nameIndex), 1);
					}
					...
					if (this.debugMode) requiredUniforms.splice(requiredUniforms.indexOf(name), 1);
			...
			if (cmd.vertexArray) {
				for (let i = 0; i < vertexLayout.length; i++) {
					...
					if (!cmd.vertexArray.attributes[name] || !cmd.vertexArray.attributes[name].location === location) {
						if (this.debugMode) console.debug(NAMESPACE, "invalid command", cmd, "vertex array doesn't satisfy vertex layout", vertexLayout);
				...
				if (cmd.vertexArray.indices) {
					...
					if (!indexBuffer && cmd.vertexArray.indices.class === "indexBuffer") indexBuffer = cmd.vertexArray.indices;
					this.state.indexBuffer = indexBuffer;
				}
			} else {
				if (this.state.vertexArray !== void 0) {
					this.state.vertexArray = void 0;
					gl.bindVertexArray(null);
				}
		...
		apply(cmd) {
			if (this.debugMode) console.debug(NAMESPACE, "apply", cmd.name || cmd.id, {
				cmd,
			...
			if (cmd.transformFeedback) this.applyTransformFeedback(cmd.transformFeedback);
