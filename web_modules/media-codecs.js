...
var vp_exports = /* @__PURE__ */ __exportAll({
...
var av_exports = /* @__PURE__ */ __exportAll({
...
const getCodec$2 = ({ name, profile: profileName, level, tier, bitDepth }) => {
	...
	if (!profile) throw new Error(`Unknown AV Profile "${profileName}"`);
	if (!AV_LEVELS.includes(level)) throw new Error(`Unknown AV Level "${level}"`);
	if (!AV_TIER.includes(tier)) throw new Error(`Unknown AV Tier "${tier}"`);
	if (!AV_BIT_DEPTH.includes(bitDepth)) throw new Error(`Unknown AV BitDepth "${bitDepth}"`);
...
var avc_exports = /* @__PURE__ */ __exportAll({
...
/** @private */
const formatLevel$1 = (level) => (parseFloat(level) * 10).toString(16).padStart(2, "0");
/** @private */
...
var hevc_exports = /* @__PURE__ */ __exportAll({
...
* @type {string[][]}
*/
const HEVC_CONSTRAINTS = Array.from({ length: 32 }, (_, i) => [(i << 3).toString(16).padStart(2, "0")]);
/**
...
const HEVC_REXT_CONSTRAINTS = (() => {
	...
	const result = [];
	for (let src = 0; src < 16; src++) for (const depth of depthNibbles) for (const [c422, c420, mono] of chromaCombos) for (let intra = 0; intra <= 1; intra++) for (let opo = 0; opo <= 1; opo++) for (let lbr = 0; lbr <= 1; lbr++) {
...
const HEVC_HT_CONSTRAINTS = (() => {
	...
	const result = [];
	for (let src = 0; src < 16; src++) for (const [depth, max14] of depthCombos) for (const [c422, c420, mono] of chromaCombos) for (let intra = 0; intra <= 1; intra++) for (let opo = 0; opo <= 1; opo++) for (let lbr = 0; lbr <= 1; lbr++) {
		const byte0 = src << 4 | depth | c422;
...
const REXT_PP = /* @__PURE__ */ new Set([
	"4",
	"6",
	"7",
	"8"
...
/** @private  */
const formatCodec = (cccc, { PP, C }, T, LL, constraints) => `${cccc}.${PP}.${C}.${T}${LL}.${constraints}`;
/**
* Return a list of all possible codec parameter string and their human readable names
...
const getAllItems = (defaultConstraints = false) => HEVC_PROFILES.map((profile) => HEVC_LEVELS.map((level) => HEVC_TIER.map((tier) => {
	...
	return (defaultConstraints ? [profile.constraints] : REXT_PP.has(profile.PP) ? HEVC_REXT_CONSTRAINTS : HT_PP.has(profile.PP) ? HEVC_HT_CONSTRAINTS : HEVC_CONSTRAINTS).map((constraints) => {
...
const getCodec = ({ profile: profileName, level, tier, constraints }) => {
	...
	if (!HEVC_LEVELS.includes(level)) throw new Error(`Unknown HEVC Level "${level}"`);
	if (!HEVC_TIER.includes(tier)) throw new Error(`Unknown HEVC Tier "${tier}"`);
...
export { av_exports as AV, avc_exports as AVC, hevc_exports as HEVC, vp_exports as VP };