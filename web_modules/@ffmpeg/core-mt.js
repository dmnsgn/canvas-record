import { c as Buffer, d as init__polyfill_node_process, f as _polyfill_node_global_default, l as init__polyfill_node_buffer, p as init__polyfill_node_global, u as browser$1 } from "../_chunks/polyfills-bhUCZgV4.js";
...
var createFFmpegCore = (() => {
	...
	return (async function(createFFmpegCore = {}) {
		...
		if (ENVIRONMENT_IS_NODE) {
			const { createRequire } = await import("../_chunks/polyfills-bhUCZgV4.js").then((n) => n.t);
			...
			if (!Module["thisProgram"] && browser$1.argv.length > 1) thisProgram = browser$1.argv[1].replace(/\\/g, "/");
			...
			_polyfill_node_global_default.Worker = nodeWorkerThreads.Worker;
		...
		if (ENVIRONMENT_IS_NODE) {
			if (typeof performance == "undefined") _polyfill_node_global_default.performance = require("perf_hooks").performance;
		}
		...
		} else wasmBinaryFile = new URL("ffmpeg-core.wasm", import.meta.url).href;
		...
		function getBinaryPromise(binaryFile) {
			if (!wasmBinary && (ENVIRONMENT_IS_WEB || ENVIRONMENT_IS_WORKER)) {
				if (typeof fetch == "function" && !isFileURI(binaryFile)) return fetch(binaryFile, { credentials: "same-origin" }).then((response) => {
					if (!response["ok"]) throw "failed to load wasm binary file at '" + binaryFile + "'";
					return response["arrayBuffer"]();
				}).catch(() => getBinary(binaryFile));
				else if (readAsync) return new Promise((resolve, reject) => {
		...
		var PThread = {
			...
			loadWasmModuleToAllWorkers: function(onMaybeReady) {
				...
				Promise.all(PThread.unusedWorkers.map(PThread.loadWasmModuleToWorker)).then(onMaybeReady);
			},
			allocateUnusedWorker: function() {
				var worker;
				if (!Module["locateFile"]) worker = new Worker(new URL("ffmpeg-core.worker.js", import.meta.url));
				else {
		...
		function establishStackSpace() {
			...
			var stackLow = stackHigh - HEAP32[pthread_ptr + 56 >> 2];
		...
		function ___pthread_kill_js(thread, signal) {
			if (signal === 33) if (!ENVIRONMENT_IS_PTHREAD) cancelThread(thread);
			else postMessage({
				"cmd": "cancelThread",
				"thread": thread
			});
		...
		function ___syscall_accept4(fd, addr, addrlen, flags, d1, d2) {
			...
			try {
				...
				if (addr) writeSockaddr(addr, newsock.family, DNS.lookup_name(newsock.daddr), newsock.dport, addrlen);
		...
		function ___syscall_getpeername(fd, addr, addrlen, d1, d2, d3) {
			...
			try {
				...
				writeSockaddr(addr, sock.family, DNS.lookup_name(sock.daddr), sock.dport, addrlen);
				return 0;
			} catch (e) {
				if (typeof FS == "undefined" || !(e.name === "ErrnoError")) throw e;
				return -e.errno;
		...
		function ___syscall_getsockname(fd, addr, addrlen, d1, d2, d3) {
			...
			try {
				var sock = getSocketFromFD(fd);
				writeSockaddr(addr, sock.family, DNS.lookup_name(sock.saddr || "0.0.0.0"), sock.sport, addrlen);
				return 0;
			} catch (e) {
				if (typeof FS == "undefined" || !(e.name === "ErrnoError")) throw e;
				return -e.errno;
		...
		function ___syscall_recvfrom(fd, buf, len, flags, addr, addrlen) {
			...
			try {
				...
				if (!msg) return 0;
				if (addr) writeSockaddr(addr, sock.family, DNS.lookup_name(msg.addr), msg.port, addrlen);
		...
		function __emscripten_thread_mailbox_await(pthread_ptr) {
			if (typeof Atomics.waitAsync === "function") {
				Atomics.waitAsync(HEAP32, pthread_ptr >> 2, pthread_ptr).value.then(checkMailbox);
		...
		function __gmtime_js(time, tmPtr) {
			var date = /* @__PURE__ */ new Date(readI53FromI64(time) * 1e3);
		...
		if (ENVIRONMENT_IS_NODE) _polyfill_node_global_default.performance = require("perf_hooks").performance;
		...
		function _emscripten_receive_on_main_thread_js(index, numCallArgs, args) {
			...
			return (!(index < 0) ? proxiedFunctionTable[index] : ASM_CONSTS[-index - 1]).apply(null, emscripten_receive_on_main_thread_js_callArgs);
		}
		...
		var _ffmpeg = Module["_ffmpeg"] = function() {
			return (_ffmpeg = Module["_ffmpeg"] = Module["asm"]["La"]).apply(null, arguments);
		};
		...
		var _emscripten_builtin_memalign = function() {
			return (_emscripten_builtin_memalign = Module["asm"]["Oa"]).apply(null, arguments);
		};
		var __emscripten_thread_init = Module["__emscripten_thread_init"] = function() {
			return (__emscripten_thread_init = Module["__emscripten_thread_init"] = Module["asm"]["Pa"]).apply(null, arguments);
		};
		var __emscripten_thread_crashed = Module["__emscripten_thread_crashed"] = function() {
			return (__emscripten_thread_crashed = Module["__emscripten_thread_crashed"] = Module["asm"]["Qa"]).apply(null, arguments);
		};
		...
		var __emscripten_run_in_main_runtime_thread_js = function() {
			return (__emscripten_run_in_main_runtime_thread_js = Module["asm"]["Sa"]).apply(null, arguments);
		};
		...
		var __emscripten_thread_free_data = function() {
			return (__emscripten_thread_free_data = Module["asm"]["Ta"]).apply(null, arguments);
		};
		var __emscripten_thread_exit = Module["__emscripten_thread_exit"] = function() {
			return (__emscripten_thread_exit = Module["__emscripten_thread_exit"] = Module["asm"]["Ua"]).apply(null, arguments);
		};
		var __emscripten_check_mailbox = Module["__emscripten_check_mailbox"] = function() {
			return (__emscripten_check_mailbox = Module["__emscripten_check_mailbox"] = Module["asm"]["Va"]).apply(null, arguments);
		};
		var _setThrew = function() {
			return (_setThrew = Module["asm"]["Wa"]).apply(null, arguments);
		};
		var _emscripten_stack_set_limits = function() {
			return (_emscripten_stack_set_limits = Module["asm"]["Xa"]).apply(null, arguments);
		};
		...
		Module["_ff_h264_cabac_tables"] = 1534124;
		Module["___start_em_js"] = 6030565;
		Module["___stop_em_js"] = 6030742;
