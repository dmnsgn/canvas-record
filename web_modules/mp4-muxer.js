...
var mp4Muxer = createCommonjsModule(function (module) {
  ...
  var Mp4Muxer = (() => {
    ...
    var moov = (tracks, creationTime) => box("moov", null, [mvhd(creationTime, tracks), ...tracks.map(x => trak(x, creationTime))]);
    var mvhd = (creationTime, tracks) => {
      ...
      return fullBox("mvhd", 0, 0, [u32(creationTime),
      // Creation time
      u32(creationTime),
      // Modification time
    ...
    var tkhd = (track, creationTime) => {
      ...
      return fullBox("tkhd", 0, 3, [u32(creationTime),
      // Creation time
      u32(creationTime),
      // Modification time
      u32(track.id),
    ...
    var mdhd = (track, creationTime) => {
      ...
      return fullBox("mdhd", 0, 0, [u32(creationTime),
      // Creation time
      u32(creationTime),
      // Modification time
      u32(track.timescale),
    ...
    var stbl = track => box("stbl", null, [stsd(track), stts(track), stss(track), stsc(track), stsz(track), stco(track)]);
    var stsd = track => fullBox("stsd", 0, 0, [u32(1)
    // Entry count
    ], [track.info.type === "video" ? videoSampleDescription(track.info.codec === "avc" ? "avc1" : "hvc1", track, track.info.codec === "avc" ? avcC(track) : hvcC(track)) : soundSampleDescription("mp4a", track, esds(track))]);
    var videoSampleDescription = (compressionType, track, child) => box(compressionType, [Array(6).fill(0),
    // Reserved
    u16(1),
    ...
    var soundSampleDescription = (compressionType, track, child) => box(compressionType, [Array(6).fill(0),
    // Reserved
    u16(1),
    // Data reference index
    u16(0),
    ...
    // length
    ...track.codecPrivate, u32(109084800),
    // TAG(6)
    u8(1),
    // length
    u8(2)
    ...
    var stco = track => {
      if (track.writtenChunks.length > 0 && last(track.writtenChunks).offset >= __pow(2, 32)) {
        return fullBox("co64", 0, 0, [u32(track.writtenChunks.length),
        // Number of entries
      ...
      return fullBox("stco", 0, 0, [u32(track.writtenChunks.length),
      // Number of entries
    ...
    addSampleToTrack_fn = function (track, data, type, timestamp, duration, meta) {
      ...
      if (!track.currentChunk || timestampInSeconds - track.currentChunk.startTimestamp >= MAX_CHUNK_DURATION) {
        if (track.currentChunk) __privateMethod(this, _writeCurrentChunk, writeCurrentChunk_fn).call(this, track);
        track.currentChunk = {
...
var __pika_web_default_export_for_treeshaking__ = /*@__PURE__*/getDefaultExportFromCjs(mp4Muxer);

var ArrayBufferTarget = mp4Muxer.ArrayBufferTarget;
var FileSystemWritableFileStreamTarget = mp4Muxer.FileSystemWritableFileStreamTarget;
var Muxer = mp4Muxer.Muxer;
var StreamTarget = mp4Muxer.StreamTarget;
export default __pika_web_default_export_for_treeshaking__;
export { ArrayBufferTarget, FileSystemWritableFileStreamTarget, Muxer, StreamTarget, mp4Muxer as __moduleExports };
